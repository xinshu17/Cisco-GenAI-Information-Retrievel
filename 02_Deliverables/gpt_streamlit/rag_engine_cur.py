...
def load_documents():
    ...
    (This function is not used in the current version; using load_documents_tesseract() instead.)
    """
...
def query_llm(retriever, query, selected_primer="You are a helpful assistant. Extract the information from this document. Say you do not know if you do not know."):
    ...
    qa_chain = ConversationalRetrievalChain.from_llm(
        llm=ChatOpenAI(model_name='gpt-4-1106-preview', openai_api_key=st.session_state.openai_api_key),
        retriever=retriever,
...
def extract_attributes_chain(retriever, output_parser=StrOutputParser()):
    '''
    Function to extract the attributes using the conversational retrieval chain.
    retriever: the retriever object
    '''
    results = []
    ...
    template = """Please extract and compile all unique information related to the following attributes from the document, avoiding any duplication. For each attribute, include the specific source or context for each unique value. Present the findings of each attribute in a single dictionary entry, where each unique value is accompanied by its corresponding source. Exclude any repeated or redundant information. Based on the document, answer the question: What is the {question} of this product? Be accurate and concise. Say you do not know if you do not know the answer."""
    prompt = ChatPromptTemplate.from_template(template)
    
    response = model.batch([{'question': 'Name', 'chat_history': st.session_state.messages},
                            {'question': 'Product Type', 'chat_history': st.session_state.messages},
                            {'question': 'Dimensions', 'chat_history': st.session_state.messages},
                            {'question': 'Orientation', 'chat_history': st.session_state.messages},
                            {'question': 'Current Rating', 'chat_history': st.session_state.messages},
                            {'question': 'Voltage', 'chat_history': st.session_state.messages},
                            {'question': 'Frequency', 'chat_history': st.session_state.messages},
                            {'question': 'Impedance', 'chat_history': st.session_state.messages},
                            {'question': 'Capacitance', 'chat_history': st.session_state.messages},
                            {'question': 'Temperature', 'chat_history': st.session_state.messages}
                            ])
    answer = [r['answer'] for r in response]
    print(answer)
    results = [(i,j) for i,j in zip(["Name", "Product Type", "Dimensions","Orientation","Current Rating","Voltage", "Frequency", "Impedance", "Capacitance", "Temperature"], answer)]
    return results
    ...
    if not st.session_state.openai_api_key or not st.session_state.source_docs:
        ...
        try:
            for source_doc in st.session_state.source_docs:
                ...
                attributes_list = ["Name", "Product Type", "Supplier Name", "Dimensions", "Orientation", "Current Rating", "Voltage", "Frequency", "Impedance", "Capacitance", "Temperature"]
                ...
                results = extract_attributes_chain(st.session_state.retriever, attributes_list)
                df = pd.DataFrame(results)
                df.rename(columns={df.columns[0]: 'attribute', df.columns[1]: 'value'}, inplace=True)
...
def load_files_from_directory(directory_path):
    '''
    Function to load the files from the directory.
